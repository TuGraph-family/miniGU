...
impl Value {
    ...
    pub fn to_i8(&self) -> Result<i8, ConversionError> {
        match self {
            ...
            Value::String(s) => s.parse::<i8>().map_err(|_| ConversionError::ParseError(s.clone())),
    ...
    pub fn to_i16(&self) -> Result<i16, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1 } else { 0 }),
            Value::String(s) => s.parse::<i16>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_i32(&self) -> Result<i32, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1 } else { 0 }),
            Value::String(s) => s.parse::<i32>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_i64(&self) -> Result<i64, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1 } else { 0 }),
            Value::String(s) => s.parse::<i64>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_u8(&self) -> Result<u8, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1 } else { 0 }),
            Value::String(s) => s.parse::<u8>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_u16(&self) -> Result<u16, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1 } else { 0 }),
            Value::String(s) => s.parse::<u16>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_u32(&self) -> Result<u32, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1 } else { 0 }),
            Value::String(s) => s.parse::<u32>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_u64(&self) -> Result<u64, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1 } else { 0 }),
            Value::String(s) => s.parse::<u64>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_f32(&self) -> Result<f32, ConversionError> {
        match self {
            ...
            Value::String(s) => s.parse::<f32>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
    ...
    pub fn to_f64(&self) -> Result<f64, ConversionError> {
        match self {
            ...
            Value::Boolean(v) => Ok(if *v { 1.0 } else { 0.0 }),
            Value::String(s) => s.parse::<f64>().map_err(|_| ConversionError::ParseError(s.clone())),
            Value::Null => Err(ConversionError::NullValue),
        }
    }
...
#[cfg(test)]
mod tests {
    use super::{Value, ConversionError};
    ...
    fn test_to_i8() {
        ...
        // Overflow cases
        assert!(matches!(Value::Int16(128).to_i8(), Err(ConversionError::Overflow))); // > i8::MAX
        assert!(matches!(Value::Int16(-129).to_i8(), Err(ConversionError::Overflow))); // < i8::MIN
        assert!(matches!(Value::UInt8(128).to_i8(), Err(ConversionError::Overflow)));
        assert!(matches!(Value::Float32(128.0).to_i8(), Err(ConversionError::Overflow)));
        assert!(matches!(Value::Float64(-129.0).to_i8(), Err(ConversionError::Overflow)));

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_i8(), Err(ConversionError::ParseError(_))));
        assert!(matches!(Value::String("128".to_string()).to_i8(), Err(ConversionError::ParseError(_)))); // Overflow via parse

        // Null
        assert!(matches!(Value::Null.to_i8(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_i16() {
        ...
        // Overflow cases
        assert!(matches!(Value::Int32(32768).to_i16(), Err(ConversionError::Overflow))); // > i16::MAX
        assert!(matches!(Value::Int32(-32769).to_i16(), Err(ConversionError::Overflow))); // < i16::MIN
        assert!(matches!(Value::UInt16(32768).to_i16(), Err(ConversionError::Overflow)));
        assert!(matches!(Value::Float64(32768.0).to_i16(), Err(ConversionError::Overflow)));

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_i16(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_i16(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_i32() {
        ...
        assert_eq!(Value::String("-2147483648".to_string()).to_i32(), Ok(-2147483648i32)); // Min value

        // Overflow cases
        assert!(matches!(Value::Int64(2147483648i64).to_i32(), Err(ConversionError::Overflow))); // > i32::MAX
        assert!(matches!(Value::Int64(-2147483649i64).to_i32(), Err(ConversionError::Overflow))); // < i32::MIN
        assert!(matches!(Value::UInt32(2147483648u32).to_i32(), Err(ConversionError::Overflow)));
        assert!(matches!(Value::Float64(2147483648.0).to_i32(), Err(ConversionError::Overflow)));

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_i32(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_i32(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_i64() {
        ...
        assert_eq!(Value::String("-9223372036854775808".to_string()).to_i64(), Ok(-9223372036854775808i64)); // Min value

        // Overflow cases
        assert!(matches!(Value::UInt64(9223372036854775808u64).to_i64(), Err(ConversionError::Overflow))); // > i64::MAX
        assert!(matches!(Value::Float64(f64::MAX).to_i64(), Err(ConversionError::Overflow))); // i64::MAX
        assert!(matches!(Value::Float64(f64::MIN).to_i64(), Err(ConversionError::Overflow))); //  i64::MIN
        assert!(matches!(Value::Float32(f32::MAX).to_i64(), Err(ConversionError::Overflow))); //  i64::MAX as f32

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_i64(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_i64(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_u8() {
        ...
        // Overflow cases
        assert!(matches!(Value::Int8(-1).to_u8(), Err(ConversionError::Overflow))); // Negative
        assert!(matches!(Value::Int16(256).to_u8(), Err(ConversionError::Overflow))); // > u8::MAX
        assert!(matches!(Value::Float32(-0.1).to_u8(), Err(ConversionError::Overflow))); // Negative float
        assert!(matches!(Value::Float64(256.0).to_u8(), Err(ConversionError::Overflow)));

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_u8(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_u8(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_u16() {
        ...
        // Overflow cases
        assert!(matches!(Value::Int16(-1).to_u16(), Err(ConversionError::Overflow))); // Negative
        assert!(matches!(Value::UInt32(65536).to_u16(), Err(ConversionError::Overflow))); // > u16::MAX
        assert!(matches!(Value::Float64(-0.1).to_u16(), Err(ConversionError::Overflow)));

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_u16(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_u16(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_u32() {
        ...
        assert_eq!(Value::String("4294967295".to_string()).to_u32(), Ok(4294967295u32)); // Max value

        // Overflow cases
        assert!(matches!(Value::Int32(-1).to_u32(), Err(ConversionError::Overflow))); // Negative
        assert!(matches!(Value::UInt64(4294967296u64).to_u32(), Err(ConversionError::Overflow))); // > u32::MAX
        assert!(matches!(Value::Float32(-0.1).to_u32(), Err(ConversionError::Overflow)));

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_u32(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_u32(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_u64() {
        ...
        assert_eq!(Value::String("18446744073709551615".to_string()).to_u64(), Ok(18446744073709551615u64)); // Max value

        // Overflow cases
        assert!(matches!(Value::Int64(-1).to_u64(), Err(ConversionError::Overflow))); // Negative
        assert!(matches!(Value::Float64(-0.1).to_u64(), Err(ConversionError::Overflow))); // Negative float
        assert!(matches!(Value::Float64(f64::MAX).to_u64(), Err(ConversionError::Overflow))); //  u64::MAX
        assert!(matches!(Value::Float32(f32::MAX).to_u64(), Err(ConversionError::Overflow))); //  u64::MAX as f32

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_u64(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_u64(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_f32() {
        ...
        // Overflow cases
        assert!(matches!(Value::Float64(f64::MAX).to_f32(), Err(ConversionError::Overflow))); // f64 max > f32 max
        assert!(matches!(Value::Float64(f64::MIN).to_f32(), Err(ConversionError::Overflow))); // f64 min < f32 min

        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_f32(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_f32(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_f64() {
        ...
        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_f64(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_f64(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_bool() {
        ...
        // Parse error
        assert!(matches!(Value::String("invalid".to_string()).to_bool(), Err(ConversionError::ParseError(_))));
        assert!(matches!(Value::String("yes".to_string()).to_bool(), Err(ConversionError::ParseError(_))));

        // Null
        assert!(matches!(Value::Null.to_bool(), Err(ConversionError::NullValue)));
    }
    ...
    fn test_to_string() {
        ...
        assert_eq!(Value::String("hello".to_string()).to_string(), Ok("hello".to_string()));

        // No overflow or parse errors for to_string, as it handles all primitives

        // Null
        assert!(matches!(Value::Null.to_string(), Err(ConversionError::NullValue)));
    }
}
