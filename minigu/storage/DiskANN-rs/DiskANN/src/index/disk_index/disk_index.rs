...
use crate::model::{
    GRAPH_SLACK_FACTOR, IndexConfiguration, MAX_PQ_CHUNKS, MAX_PQ_TRAINING_SET_SIZE,
    ...
    [T; N]: FullPrecisionDistance<T, N>,
{
    ...
    fn fetch_disk_build_param(&self) -> ANNResult<&DiskIndexBuildParameters> {
        self.disk_build_param
            .as_ref()
            .ok_or_else(|| ANNError::IndexConfigError {
                parameter: "disk_build_param".to_string(),
                err: "disk_build_param is None".to_string(),
            })
    }
}
