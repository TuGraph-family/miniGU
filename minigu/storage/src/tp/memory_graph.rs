...
pub mod tests {
    ...
    mod import_and_export {
        ...
        // label_id -> label_set
        fn find_label_set_by_label_id(
            label_id: LabelId,
            label_set_list: &Vec<LabelSet>,
        ) -> Option<&LabelSet> {
            label_set_list.iter().find(|&ls| ls.contains(label_id))
        }
        ...
        impl MemoryGraph {
            ...
            pub fn import<P: AsRef<Path>>(
                ...
                let config_str = fs::read_to_string(config_path.as_ref()).unwrap();
                ...
                for vertex_config in config.vertex_config_list.iter() {
                    ...
                    let label_id = schema.get_label_id(&vertex_config.label).unwrap().unwrap();

                    for record in rdr.records() {
                        let label_set_list = schema.vertex_type_keys();
                        let label_set =
                            find_label_set_by_label_id(label_id, &label_set_list).unwrap();
                        let props = schema
                ...
                for edge_config in config.edge_config_list.iter() {
                    let label_id = schema.get_label_id(&edge_config.label).unwrap().unwrap();
                    ...
                    for record in rdr.records() {
                        let label_set_list = schema.edge_type_keys();
                        let label_set =
                            find_label_set_by_label_id(label_id, &label_set_list).unwrap();
                        ...
                        let edge =
                            Edge::new(eid, src_id, dst_id, label_id, PropertyRecord::new(props));
            ...
            pub fn export<P: AsRef<Path>>(
                ...
                let (label_id_to_label, mut file_map): (HashMap<_, _>, HashMap<_, _>) = schema
                    .label_names()
                    .iter()
                    .map(|label| {
                        let filename = format!("{label}.csv");
                        let path = dir.join(&filename);
                        let label_id = schema.get_label_id(label).unwrap().unwrap();
        ...
        fn mock_schema() -> MemoryGraphTypeCatalog {
            let mut schema = MemoryGraphTypeCatalog::new();
            let person_id = schema.add_label("person".to_string()).unwrap();
            let friend_id = schema.add_label("friend".to_string()).unwrap();
            let follow_id = schema.add_label("follow".to_string()).unwrap();
            ...
            schema.add_vertex_type(person_label_set, vertex_type.clone());
