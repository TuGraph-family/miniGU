...
impl PQStorage {
    ...
    pub fn load_pivot_data(
        ...
        if pivot_num != *num_centers || pivot_dim != *dim {
            let error_message = format!(
                "Error reading pq_pivots file {}. file_num_centers = {}, file_dim = {} but expecting {} centers in {} dimensions.",
                &self.pivot_file, pivot_num, pivot_dim, num_centers, dim
            );
            return Err(ANNError::log_pq_error(error_message));
        }
        ...
        if centroid_dim != *dim || nc != 1 {
            let error_message = format!(
                ...
                &self.pivot_file, centroid_dim, nc, dim
            );
            return Err(ANNError::log_pq_error(error_message));
        }
        ...
        if chunk_offset_number != *num_pq_chunks + 1 || nc != 1 {
            let error_message = format!(
                "Error reading pq_pivots file at chunk offsets; file has nr={}, nc={} but expecting nr={} and nc=1.",
                chunk_offset_number,
                nc,
                num_pq_chunks + 1
            );
...
mod pq_storage_tests {
    ...
    fn write_compressed_pivot_data_test() {
        ...
        let compressed_base: Vec<usize> = (0..block_size * num_pq_chunks)
            .map(|_| rng.random_range(0..num_centers))
            .collect();
